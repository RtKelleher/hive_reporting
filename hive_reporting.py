...
Todo:
     * Remove manual aspect of Panda Charts
...
class SIRPPipeline(object):
    ...
    data_frame_INDEX = [
        'Created', 'Severity', 'Owner', 'Name', 'Closed', 'Resolution'
    ]
    ...
    def _add_record(days_dict, record, key):
        ...
        days_dict[key] = {
            ...
            record["severity"],
            "Created":
            (time.strftime(SIRPPipeline.TIME_FMT,
    ...
    def make_dataframes(self):
        ...
        """
        self._data_frame_30days = pd.DataFrame(
            self._all30_dict, index=SIRPPipeline.data_frame_INDEX).transpose()
        self._data_frame_60days = pd.DataFrame(
            self._all60_dict, index=SIRPPipeline.data_frame_INDEX).transpose()
        self._data_frame_90days = pd.DataFrame(
            self._all90_dict, index=SIRPPipeline.data_frame_INDEX).transpose()
        counts = self._data_frame_30days['Severity'].value_counts().to_dict()
        self._data_frame_counts = pd.DataFrame({
            'Created': {
                'total': self._data_frame_30days.count()['Created']
            },
            'Closed': {
                'total': self._data_frame_30days.count()['Closed']
            },
            'Owner':
            (self._data_frame_30days['Owner'].value_counts().to_dict()),
            'Resolution':
            (self._data_frame_30days['Resolution'].value_counts().to_dict()),
            'Severity': {k: counts.get(k, 0)
                         for k in range(1, 4)}
        })
    ...
    def make_charts(self):
        ...
        def _data_frame_days_to_excel(writer, sheet_name, data_frame_days):
            data_frame_days.to_excel(writer,
                                     sheet_name=sheet_name,
                                     startrow=1,
                                     header=False)
            self._set_workbook_layout(writer.book, (writer.sheets[sheet_name]),
                                      data_frame_days)

        with pd.ExcelWriter("Hive Metrics.xlsx",
                            engine="xlsxwriter",
            ...
            _insert_pie_chart(workbook,
                              ...
                              series={
                                  ...
                                  'categories': '=Tracking!$A$8:$A$10',
                                  'values': '=Tracking!$F$8:$F$10',
                              })
            _insert_pie_chart(workbook,
                              worksheet,
                              ...
                              series={
                                  ...
                                  'categories': '=Tracking!$A$6:$A$7',
                                  'values': '=Tracking!$E$6:$E$7',
                              })
            _insert_pie_chart(workbook,
                              worksheet,
                              ...
                              series={
                                  ...
                                  'categories': '=Tracking!$A$3:$A$5',
                                  'values': '=Tracking!$D$3:$D$5',
                              })

            _data_frame_days_to_excel(writer,
                                      sheet_name="Cases newer than 30 Days",
                                      data_frame_days=self._data_frame_30days)
            _data_frame_days_to_excel(writer,
                                      sheet_name="Cases older than 60 days",
                                      data_frame_days=self._data_frame_60days)
            _data_frame_days_to_excel(writer,
                                      sheet_name="Cases newer than 90 Days",
                                      data_frame_days=self._data_frame_90days)
    ...
    def send_mail():
        ...
        msg.attach(
            MIMEText(
                "Attached are the requested case metrics in .XLSX format."))
        part = MIMEBase("application", "octet-stream")
        part.set_payload(open("Hive Metrics.xlsx", "rb").read())
        encoders.encode_base64(part)
        part.add_header("Content-Disposition",
                        'attachment; filename="Hive Metrics.xlsx"')
        msg.attach(part)
